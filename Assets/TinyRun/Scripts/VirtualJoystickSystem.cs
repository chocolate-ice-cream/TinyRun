    ...
    public class VirtualJoystickSystem : ComponentSystem {
        protected override void OnUpdate() {
            ...
            Entities.ForEach((Entity entity, ref Joystick joystick, ref RectTransform rectTransform) => {
                ...
                if (!joystick.IsPressedSpaceKey && input.GetKeyDown(KeyCode.Space)) {
                    joystick.IsPressedSpaceKey = true;
                    joystick.IsPressingSpaceKey = true;
                }
                else if (joystick.IsPressedSpaceKey) {
                    joystick.IsPressedSpaceKey = false;
                }

                if (!joystick.IsPressedDownArrowKey && input.GetKeyDown(KeyCode.DownArrow)) {
                    joystick.IsPressedDownArrowKey = true;
                    joystick.IsPressingDownArrowKey = true;
                }
                else {
                    joystick.IsPressedDownArrowKey = false;
                }

                if (!joystick.IsPressedUpArrowKey && input.GetKeyDown(KeyCode.UpArrow)) {
                    joystick.IsPressedUpArrowKey = true;
                    joystick.IsPressingUpArrowKey = true;
                }
                else {
